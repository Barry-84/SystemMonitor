...
func (u *Uploader) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// TODO:
	//  Temporarily support legacy clients sending File-Path request header.
	//  But after legacy clients no longer exist, simplify this code to only user query param.
	missingFilePathHeader := false
	file_path := ParseHeaderString(w, r, "File-Path", &missingFilePathHeader)
	if missingFilePathHeader {
		file_path = q.Get("file_path")
	}

	// TODO: This header is missing from legacy clients.
	//  After all client deployments have been upgraded to send "Overwrite-Mode",
	//  this code can be moved up to the required headers above, and use &fail instead of &missingOverwriteMode.
	missingOverwriteMode := false
	overwriteMode := ParseHeaderString(w, r, "X-Overwrite-Mode", &missingOverwriteMode)
	...
	log.Println("Overwrite-Mode", overwriteMode)
	...
	final_path := path.Join(upload_path, file_path)

	writeFileFlags := os.O_RDWR|os.O_APPEND|os.O_CREATE
	if chunk_number == 1 { // Handle overwriteMode
 		writeFileFlags = os.O_RDWR|os.O_CREATE|os.O_TRUNC // Create a new file (omits os.O_APPEND)
		log.Println("First chunk. writeFileFlags: ", writeFileFlags)
		if overwriteMode != "replace" {
			// Default: if file exists, respond with "failure" + "file_exists"
			fileInfo, err := os.Stat(final_path)
			if err == nil { // File exists
				log.Println("Found existing file: ", fileInfo.Name())
				response.Status = "failure"
				w.WriteHeader(409) // status code for "Conflict"
				return
			}
		}
	}
	...
	file, err := os.OpenFile(final_path, writeFileFlags, 0666)
	defer file.Close()
	if err != nil {
		pe := err.(*os.PathError)
